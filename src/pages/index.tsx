...
import { doc, collection, onSnapshot, deleteDoc, setDoc, getDoc } from "firebase/firestore";
...
const randomId = Math.random()
        ...
        await setDoc(doc(firestore, "users", `${user.uid}`, 'tasks', `${randomId}` ), {
          device_type: device,
          good_device: good,
          bad_device: bad,
...
// get data
  ...
  onSnapshot(doc(firestore, "users", `${user?.uid}`), (doc) => {
    console.log("Current data: ", doc.data()?.user_name);
});
...
<button className="p-1 border rounded w-32" onClick={showModal}>Cancel</button>
...
<table className="table-auto w-full px-4 css-serial">
  ...
  <tbody className=' w-full body '>
    {data && data?.map(( tableData: any ) =>(
 <>
      <tr className='h-12 ' key={tableData.id}>
        <td className="relative">&nbsp;</td>
      <td className='text-start'>{tableData.device_type}</td>
      <td className='text-start'>{tableData.good_device}</td>
      <td className='text-start'>{tableData.bad_device}</td>
      <td className='text-start'>{tableData.total_device_repair}</td>
      <td className='text-start'>{tableData.date_doc_add}</td>
      <td className='text-start cursor-pointer'><FontAwesomeIcon
        icon={faTrashCan}
        style={{ fontSize: 20, color: "gray" }}
        onClick={() => deleteDocument(tableData.id)}
      /></td>
    </tr>
      </>
...
{isloading ? <div className="w-full h-52  flex justify-center items-center">
  <span className="flex items-center gap-4">
  <h1 className="text-4xl font-bold">loading records...</h1>
  </span>

</div> : data.length === 0 && <div className="w-full h-52  flex justify-center items-center">
  <span className="flex items-center gap-4">
  <h1 className="text-4xl font-bold">Add new records</h1>
    <FontAwesomeIcon
        icon={faClipboard}
